...
if(!$num){
	...
	message('添加失败','wstyle.php?id='.$row_style);
	
	}else{
     message('添加成功','wstyle.php?id='.$row_style);
		// header("Location: ./index.php");

}
}else{
	echo "<script>alert('宝贝已经在购物车等你啦！');parent.location.href='wstyle.php?id=$row_style'; </script>";
}
?>