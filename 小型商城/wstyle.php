<?php
 ...
 //分页
  ...
  $pagesize=32;
 $sql_count="select count(*)  as count from mz_goods where style_no=$id and goods_sex=1 ";
 ...
 for($i=1;$i<=$maxpage;$i++)
 $maxarray[$i]=$i;
 $sql="select * from mz_goods where style_no=$id and goods_sex=1 limit ".($page-1)*$pagesize.",$pagesize";
 $wgoods=my_query($sql);
//收藏夹商品
 ...
 foreach($wgoods as $key=>$v)
   { 
     $sql="select * from mz_style where style_sex=1 and style_no=".$v['style_no'];
       $style=my_query($sql);
     $wgoods[$key]['stylename']=$style[0]['style_name'];
 }
$smarty->assign("id",$id);
 ...
 $smarty->assign('wgoods',$wgoods);
  $smarty->display("templates/wstyle.html");
?>